...
def structure(code, path: tuple, mesh, call=False):
	...
	if code.startswith('->'):
		...
		if call:
			if code.is_in(')'):
				mesh[path[:-1]] = [path, path[-1]]
				#kwd = 'self'
			else:
				mesh[path[:-1]] = [path, path[-1], variable(code)]
				#kwd = variable(code)
		else:
			any(code, path+('self',), mesh, call)
	else:
		if call:
			mesh[path[:-1]] = [path, path[-1], 'self']
		else:
...
def seek(mesh, path):
	...
	for i in reversed(range(len(path))):
		...
		for oldpath in mesh.copy():
			...
			if mesh[newsubpath]:
				mesh[subpath] = chroot(mesh[newsubpath], subpath, newsubpath)
