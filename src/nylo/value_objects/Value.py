...
class Value(Token):
    ...
    def parse(self, reader):
        ...
        if reader.any_starts_with(Keyword.starts): self.value = Keyword(reader)
        
        elif reader.any_starts_with(Number.starts): self.value = Number(reader)
        
        elif reader.any_starts_with(String.starts): self.value = String(reader)
        
        elif reader.any_starts_with(Struct.starts): self.value = Struct(reader)
            
        elif reader.read() == '\0': self.value = ValueLayer(None
            
        else: unx_char(reader.read())
        
        if reader.read() == '(': self.value = Caller(copy.copy(self), reader)
